...
<head>
    ...
    <title>EVI Request Public Event</title>
...
<body>
    ...
    <?php 
        if(isset($_POST['create']))
        {
            ...
            $des = $_POST['event_des'];
            $university = $_SESSION['current_uni'];

            #echo "$name <br> $time <br> $date <br> $cat <br> $email <br> $phone <br> $des <br>";

            $query = "INSERT INTO `public_requests`(`admin_name`, `admin_email`, `admin_phone`, `university`, `name`, `date`, `time_from`, `time_to`, `category`, `description`, `location`) VALUES ('$admin_name', '$email', '$phone', '$university', '$name', '$date', '$start_time', '$end_time', '$cat', '$des', '$location')";
            mysqli_query($con, $query);
            echo "<script>
                    window.location.href='admin_public.php'; 
                  </script> ";
        }
    ?>
